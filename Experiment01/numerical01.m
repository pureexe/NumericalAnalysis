...
f = @(x) sin(x);
H = @(x) 2^-(x); % interested H function
forward_difference = @(x,h) (f(x+h) - f(x)) / h;
backward_difference = @(x,h) (f(x) - f(x-h)) / h;
...
for i = 1:8
    ...
    fprintf("%d  %4.4e  %4.4e  %4.4e  %4.4e  %4.4e\n", ...
        i, ...
        absolute_error(forward_difference(x,h)), ...
        absolute_error(backward_difference(x,h)), ...
        absolute_error(center_difference(x,h)), ...
        absolute_error(three_point_difference(x,h)), ...
        absolute_error(richard_o4_difference(x,h)) ...
     );
end